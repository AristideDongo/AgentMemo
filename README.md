...
[![GitHub stars](https://img.shields.io/github/stars/AristideDongo/IA-multi-session?style=flat&logo=github)](https://github.com/AristideDongo/IA-multi-session)

**Passe d’une IA à une autre sans perdre le fil du projet.**

`aihub` est un CLI interactif qui conserve une mémoire locale dans ton projet : objectif, tâches, décisions et passages de relais. Le prochain agent retrouve le contexte dans `AGENTS.md`, par une commande ou via le serveur MCP local.
...
git clone https://github.com/AristideDongo/IA-multi-session.git
cd IA-multi-session
npm ci
npm run build
npm link
...
`npm link` rend la commande `aihub` disponible dans ton environnement Node. Elle pointe vers la version compilée de ce dépôt. Après une modification des sources, relance `npm run build`.
...
4. `aihub` ouvre le menu interactif.

Pour accompagner le dépôt aihub lui-même pendant le développement :
...
```bash
aihub --project /chemin/vers/ton-projet init
aihub --project /chemin/vers/ton-projet interactive
aihub --project /chemin/vers/ton-projet status
```
...
Lance `aihub` sans argument dans un terminal, ou `aihub interactive` :
...
L’interactivité nécessite un terminal en entrée et en sortie. Sans terminal, `aihub` sans argument affiche l’aide. `init --json` ne pose aucune question, ce qui convient aux scripts.

## Référence des commandes

```text
aihub [--project chemin] <commande> [options]
```
...
```bash
aihub task add -- "--titre littéral"
aihub decision add "Convention" --reason="--option est intentionnel"
```
...
```bash
aihub init
aihub init --objective "Fiabiliser les webhooks"
aihub init --objective "Fiabiliser les webhooks" --json
aihub objective "Préparer la prochaine version"
```
...
```bash
aihub task add "Écrire les tests"
aihub task add "Corriger le parseur" --doing
aihub task list
aihub task list --status doing --json
aihub task start T1
aihub task done T1 --note "Tests réussis"
aihub task reopen T1 --note "Cas supplémentaire à vérifier"
```
...
```bash
aihub decision add "Utiliser Redis" --reason "Garantir l’idempotence"
aihub decision list
aihub decision list --json
```
...
aihub decision add "Vérifier la signature avant le traitement"
aihub task done T1 --note "Tests de signature verts"
aihub task add "Tester le rejeu après expiration"
aihub handoff --agent dev --summary "Validation terminée" \
...
aihub ne transmet aucune donnée à un service distant. Un agent qui lit le contexte peut néanmoins le transmettre à son propre fournisseur. Une sauvegarde du projet doit inclure explicitement les fichiers ignorés si tu souhaites conserver cette mémoire.
...
```json
{
  ...
  "args": [
    "/chemin/absolu/IA-multi-session/dist/cli.js",
    "--project",
...
| --- | --- | --- |
| `aihub_context` | Aucun | Aucun |
| `aihub_task_add` | `title` | Aucun |
| `aihub_task_update` | `id`, `status` (`todo`, `doing`, `done`) | `note` |
| `aihub_decision_add` | `title` | `reason` |
| `aihub_handoff` | `agent`, `summary` | `next`, `blockers` et `files` (listes de chaînes) |
...
| Symptôme | Solution |
| --- | --- |
| `aihub: command not found` | Depuis le dépôt du CLI, lance `npm run build` puis `npm link`, ou utilise `node /chemin/vers/dist/cli.js`. |
| `Projet non initialisé` | Place-toi dans le projet puis lance `aihub init`, ou précise `--project`. |
...
| `Tâche T1 introuvable` | Consulte `aihub task list` et utilise l’identifiant existant. |
| Le menu ne s’affiche pas | Lance `aihub` dans un terminal, sans redirection ni `--json`. |
| Les dernières modifications du CLI ne sont pas visibles | Relance `npm run build` ou utilise `npm run dev`. |
| Le contexte n’est pas à jour | Lance `aihub sync` dans le bon projet. |
...
Après un arrêt brutal, le dossier peut rester présent. **Vérifie qu’aucun processus aihub n’utilise le projet** avant de retirer le dossier vide depuis sa racine :
...
Une modification peut réussir dans `state.json` puis échouer sur `AGENTS.md`, par exemple faute de permission. Si le message indique que l’état a déjà été enregistré, corrige l’accès au fichier puis lance `aihub sync`, plutôt que de répéter l’ajout et créer un doublon.
